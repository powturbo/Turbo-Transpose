...
int main(int argc, char* argv[]) {
  unsigned trips = 15,cmp=0, b = 1 << 30, esize=4, lz=0, fno;
  
  int c, digit_optind = 0, this_option_optind = optind ? optind : 1, option_index = 0;
  static struct option long_options[] = { {"blocsize", 	0, 0, 'b'}, {0,0, 0, 0}  };
  for(;;) {
    if((c = getopt_long(argc, argv, "b:ci:I:s:z", long_options, &option_index)) == -1) break;
    switch(c) {
  ...
  for(fno = optind; fno < argc; fno++) {
    ...
    if(!lz) {
      TMBENCH("tpenc",tpenc(in, n,out,esize) ,n);					  	  TMBENCH("",tpdec(out,n,cpy,esize) ,n);  						memcheck(in,n,cpy); printf("\n");	  
      TMBENCH("tp4enc",tp4enc(in,n,out,esize) ,n);      				  TMBENCH("",tp4dec(out,n,cpy,esize) ,n);  					memcheck(in,n,cpy); printf("\n");	   
        #ifdef BLOSC
      TMBENCH("blosc shuffle",shuffle(esize,n,in,out), n);				  TMBENCH("",unshuffle(esize,n,out,cpy), n); 					memcheck(in,n,cpy); printf("\n");	  
        #endif
        #ifdef BITSHUFFLE
      TMBENCH("bitshuffle",bshuf_bitshuffle(in,out,(n)/esize,esize,0), n); TMBENCH("",bshuf_bitunshuffle(out,cpy,(n)/esize,esize,0), n);memcheck(in,n,cpy);printf("\n");	  
        #endif
	  TMBENCH("memcpy",memcpy(in,out,n) ,n);      printf("\n");
    }
    totlen += n;
    // Test Transpose + lz	
    char *tmp; int rc;   
    ...
    printf("compressed len=%u ratio=%.2f\n", rc, (double)(rc*100.0)/(double)n);
	  ...
	  #ifdef BITSHUFFLE
    //memrcpy(cpy,in,n); TMBENCH("bitshuffle+lz4",rc=bshuf_compress_lz4(in,out,n/esize,esize,0), n); tot[2] += rc; TMBENCH("",bshuf_decompress_lz4(out,cpy,n/esize,esize,0), n); memcheck(in,n,cpy);
