...
#define TMEND(_len_) _tm_r++; if((_tm_t = tmtime() - _tm_t0) > tm_tx) break; } \
  ...
  else if(_tm_t>tm_tm*1.2) TMSLEEP;   													if(tm_verbose) printf("%8.2f %.2d_%d\b\b\b\b\b\b\b\b\b\b\b\b\b",TMBS(_len_, (double)tm_tm/tm_rm),_tm_R+1,_tm_c),fflush(stdout);\
...
static tm_t     tm_tx = TM_T, tm_TX = 10*TM_T, tm_0, tm_T, tm_verbose=0, tm_tm;
...
#define TMBENCH(_name_, _func_, _len_)  do { if(tm_verbose) printf("%-32s: ", _name_ ); TMBEG(tm_rep, tm_Rep) _func_; TMEND(_len_); if(tm_verbose) printf("%.3f           \n", TMBS(_len_, (double)tm_tm/(double)tm_rm) ); } while(0)
#define TMBENCHF(_name_,_func_, _len_, _res_)  do { if(tm_verbose) printf("%-32s: ", _name_ ); TMBEG(tm_rep, tm_Rep) if(_func_ != _res_) { printf("ERROR: %lld != %lld", (long long)_func_, (long long)_res_ ); exit(0); }; TMEND(_len_); if(tm_verbose) printf("%.3f           \n", TMBS(_len_,(double)tm_tm/(double)tm_rm) ); } while(0)
